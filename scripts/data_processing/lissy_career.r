...
#household
h_var_list <- c("hid","year", "dhi", "nhhmem", "hwgt","rural","region_c","own","hpopwgt")
names_files_h <- c(
#France
...
#personal 
p_var_list<- c("hid",'educ',"educ_c","pid", "pi11", "age", "relation", "pwgt","immigr","inda1",'occ1_c','indb1')
names_files_p <- c(
#France
...
df$manufacturing <- df$indb1 == "[2]mining and quarrying; manufacturing; utilities"
# Group by file, region, and year and calculate average wages
df_manufacturing <- df %>%
  filter(manufacturing) %>%
  group_by(file, region = sub("^\\[\\d+\\](.*?) \\(.*\\)", "\\1", region_c), year) %>%
  summarize(avg_wage_manufacturing = weighted.mean(dhi, hwgt)) %>%
  ungroup()
df_all <- df %>%
  group_by(file, region = sub("^\\[\\d+\\](.*?) \\(.*\\)", "\\1", region_c), year) %>%
  summarize(avg_wage_all = weighted.mean(dhi, hwgt)) %>%
  ungroup()
print(head(df_manufacturing))
print(head(df_all))

# Merge the two datasets and calculate the ratio
merged_df_wages <- left_join(df_manufacturing, df_all, by = c("file", "region", "year"))